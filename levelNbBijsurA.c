...
void handle_keyboardBijA(SDL_Event e);
...
void init_lvlNbBijsurA(void) {
	...
	create_text(0,S_WIDTH/2,S_HEIGHT/8,color,font,"Nombre  de  bijections  sur  un  ensemble  A");
    create_various_texture(1,S_WIDTH/2,3*S_HEIGHT/10,S_WIDTH/2+S_WIDTH/4,S_HEIGHT/9,"images/formulas/nbBijsurA.JPG");
	...
	create_blank(0,S_WIDTH/2-S_WIDTH/5, 3*S_HEIGHT/10, S_WIDTH/3/4, S_HEIGHT/9);

	instructions = (char **) realloc(instructions, 2*sizeof(char*));
	char* instrs[2] = {"A!"," "};
	if (exoMode==1) {
        for (int i=0;i<2;i++) {
            instrs[i]=" ";
	...
	for (int j=0;j<2;j++) {
        instructions[j] = (char *) malloc(4 * sizeof(char));
...
void redirect_lvlNbBijsurA(int index, int* quit) {
	switch(index) {
		case 0:
			//init
			destroy_lvlNbBijsurA();
			if (mPause) {
...
void update_lvlNbBijsurA(int* quit, SDL_Event e) {
	while(SDL_PollEvent(&e) != 0) {
	    handle_keyboardBijA(e);
		handle_mickey_mouse(quit,&clickHandlerState,&indexButton,e,&oldIndex,redirect_lvlNbBijsurA,destroy_lvlNbBijsurA);
	}
}
...
void handle_keyboardBijA(SDL_Event e) {
    switch(e.type) {
		case SDL_KEYDOWN:
			...
			else if (e.key.keysym.sym == SDLK_EXCLAIM || e.key.keysym.sym == SDLK_KP_EXCLAM || e.key.keysym.sym == SDLK_1) {
                if (state==1 && combo == 1) {
