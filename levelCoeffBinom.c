...
void handle_keyboardCoeffBinom(SDL_Event e);
...
void redirect_lvlCoeffBinom(int index, int* quit) {
	switch(index) {
		case 0:
			//init
			destroy_lvlCoeffBinom();
			if (mPause) {
...
void update_lvlCoeffBinom(int* quit, SDL_Event e) {
	while(SDL_PollEvent(&e) != 0) {
	    handle_keyboardCoeffBinom(e);
		handle_mickey_mouse(quit,&clickHandlerState,&indexButton,e,&oldIndex,redirect_lvlCoeffBinom,destroy_lvlCoeffBinom);
	}
}
